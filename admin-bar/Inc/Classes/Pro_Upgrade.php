...
class Pro_Upgrade {
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jlt_admin_bar_editor_dashboard_widget',                          // Widget slug.
			esc_html__( 'admin-bar News & Updates', 'admin-bar' ), // Title.
		...
		$example_widget_backup = array( 'jlt_admin_bar_editor_dashboard_widget' => $default_dashboard['jlt_admin_bar_editor_dashboard_widget'] );
		unset( $default_dashboard['jlt_admin_bar_editor_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="admin-bar-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="admin-bar-dashboard-promo" style="--admin-bar-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="admin-bar Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="admin-bar-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				// $item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . esc_html__( 'ago', 'admin-bar' );
		...
		<div class="admin-bar-dashboard_footer">
			<ul>
				<li class="admin-bar-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'admin-bar' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="admin-bar-overview__help">
                ...
                <li class="admin-bar-overview__upgrade">
		...
		<style>
			...
			.admin-bar-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.admin-bar-rss-widget .admin-bar-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.admin-bar-rss-widget .admin-bar-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.admin-bar-rss-widget .admin-bar-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.admin-bar-rss-widget .admin-bar-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.admin-bar-rss-widget .admin-bar-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jlt_admin_bar_editor_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jlt_admin_bar_editor_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jlt_admin_bar_editor_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLT_ADMIN_BAR_EDITOR_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jlt_admin_bar_editor_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jlt_admin_bar_editor_sheet_promo_data', $remote_data );
			update_option( 'jlt_admin_bar_editor_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jlt_admin_bar_editor_sheet_promo_data_reset' );
		}
	}
...
", $response));
		...
		$plugin_slug = Helper::jlt_admin_bar_editor_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="admin-bar-popup admin-bar-upgrade-popup" id="admin-bar-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="admin-bar-popup-overlay"></div>

			<div class="admin-bar-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --admin-bar-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="admin-bar-popup-modal-footer">
					...
					<div class="admin-bar-popup-countdown" style="display: none;">
						<?php if(!empty( $this->get_content( 'notice' ) )){ ?>
							<span data-counter="notice" style="color:#F4B740; font-size:14px; padding-bottom:20px; font-style:italic;">
								<?php echo esc_html__( 'Notice:', 'admin-bar' );?> <?php echo $this->get_content( 'notice' );?>
							</span>
						<?php } ?>
						<span class="admin-bar-popup-countdown-text"><?php echo esc_html__( 'Offer Ends In', 'admin-bar' );?></span>
					...
					<!-- button  -->
					<a class="admin-bar-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#admin-bar-popup'),
				plugin_data = <?php echo wp_json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".admin-bar-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".admin-bar-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".admin-bar-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".admin-bar-popup-countdown").hide(0);
				}
